...
export default function Dashboard() {
  ...
  // Fetch when page changes (but not on filter reset — handled above)
  useEffect(() => {
    fetchCompanies(page)
  }, [page])
      ...
      <div style={{ flex: 1, marginLeft: '224px', display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
        ...
        <div style={{ padding: '8px 24px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: '16px' }}>
          <p style={{ fontSize: '12px', color: 'var(--text-dim)', margin: 0 }}>
            {total > 0 && !isSmartMode
              ? `${startItem}–${endItem} of ${total} companies`
              : `${total} companies`}
          </p>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0 }}>
                        <div style={{ position: 'relative', width: '32px', height: '32px' }}>
                          ...
                          <span style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px', fontWeight: 700, color: getScoreColor(c.opportunity_score) }}>
                        ...
                        <button onClick={e => toggleFavorite(e, c.id)}
                          style={{ fontSize: '20px', background: 'none', border: 'none', cursor: 'pointer', color: c.is_favorite ? '#FBBF24' : 'var(--text-dim)', transition: 'color 0.15s' }}>
                          ★
                        </button>
                      </div>
                    </div>
              ...
              {!isSmartMode && totalPages > 1 && (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', paddingTop: '16px', paddingBottom: '8px' }}>
                  ...
                  <button onClick={() => goToPage(page - 1)} disabled={page === 1}
                    style={{ width: '34px', height: '34px', borderRadius: '8px', border: '1px solid var(--border)', background: 'var(--bg-input)', color: page === 1 ? 'var(--text-dim)' : 'var(--text-muted)', cursor: page === 1 ? 'not-allowed' : 'pointer', fontSize: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: page === 1 ? 0.4 : 1, transition: 'all 0.15s' }}
                    onMouseEnter={e => { if (page !== 1) e.currentTarget.style.borderColor = 'rgba(79,123,247,0.4)' }}
                  ...
                  {getPageNumbers().map((p, i) => (
                    p === '...' ? (
                      ...
                      <button key={p} onClick={() => goToPage(p as number)}
                        style={{
                          width: '34px', height: '34px', borderRadius: '8px', fontSize: '13px', fontWeight: page === p ? 600 : 400,
                          border: page === p ? 'none' : '1px solid var(--border)',
                          background: page === p ? 'linear-gradient(135deg, #4F7BF7, #7C3AED)' : 'var(--bg-input)',
                          color: page === p ? 'white' : 'var(--text-muted)',
                          cursor: 'pointer', transition: 'all 0.15s',
                        }}
                        onMouseEnter={e => { if (page !== p) e.currentTarget.style.borderColor = 'rgba(79,123,247,0.4)' }}
                        onMouseLeave={e => { if (page !== p) e.currentTarget.style.borderColor = 'var(--border)' }}>
                        {p}
                      </button>
                    )
                  ))}
                  ...
                  <button onClick={() => goToPage(page + 1)} disabled={page === totalPages}
                    style={{ width: '34px', height: '34px', borderRadius: '8px', border: '1px solid var(--border)', background: 'var(--bg-input)', color: page === totalPages ? 'var(--text-dim)' : 'var(--text-muted)', cursor: page === totalPages ? 'not-allowed' : 'pointer', fontSize: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: page === totalPages ? 0.4 : 1, transition: 'all 0.15s' }}
                    onMouseEnter={e => { if (page !== totalPages) e.currentTarget.style.borderColor = 'rgba(79,123,247,0.4)' }}
                    onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--border)' }}>
                    →
                  </button>
