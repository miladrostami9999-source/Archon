...
- [ ] `SERPER_API_KEY` — **اختیاری ولی به‌شدت توصیه‌شده.** Lead Hunter بدون این از web search داخلی Anthropic استفاده می‌کنه که **~$۰.۵۸ برای هر ۵ شرکت** خرج برمی‌داره (اندازه‌گیری‌شده). با این کلید همون کار حدود **$۰.۰۲** می‌شه.
  - از `serper.dev` — **۲,۵۰۰ سرچ رایگان، بدون نیاز به کارت اعتباری** (بررسی‌شده تیر ۱۴۰۵). بهترین گزینه.
  - جایگزین: `BRAVE_SEARCH_API_KEY` از `brave.com/search/api` — $۵/۱۰۰۰ با $۵ کردیت رایگان ماهانه (~۱۰۰۰ سرچ)، ولی **کارت اعتباری اجباریه** حتی برای کردیت رایگان.
...
- [x] Add Company — تأیید شد (بعد از فیکس Postgres sequence)
- [x] تولید ایمیل با AI (`CLAUDE_API_KEY`) — تأیید شد
- [x] ارسال ایمیل واقعی — تأیید شد (بعد از مهاجرت SMTP→Resend)
- [ ] **تست ارسال با attachment واقعی** — هنوز تست نشده (چک کنه فایل واقعاً پیوست میشه، نه فقط لینک/متن)
- [ ] باز کردن یک شرکت مشخص، تست ادیت — هنوز تست نشده
- [ ] Public Profile (`/u/username`) بدون لاگین باز می‌شه — هنوز تست نشده
- [ ] Forgot Password → ایمیل واقعی دریافت می‌شه (این هم الان از Resend استفاده می‌کنه، باید دوباره تست بشه) — هنوز تست نشده
- [ ] Weekly Report تولید می‌شه — هنوز تست نشده
- [ ] Daily Tasks تولید می‌شه — هنوز تست نشده
