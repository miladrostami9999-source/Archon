...
- ⚠️ **سقف‌ها باید از داخل Admin Panel قابل ویرایش باشن** (درخواست Milad) — یعنی مقادیر نباید در کد hardcode بشن؛ باید در یک جدول `plan_limits` توی دیتابیس ذخیره بشن تا ادمین بتونه مثلاً سقف ایمیل پلن Pro رو کم/زیاد کنه و بلافاصله روی همه‌ی کاربرای اون پلن اثر بذاره
